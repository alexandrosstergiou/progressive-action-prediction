...
class MTNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([self.avgpool(x_short),self.avgpool(x_long)],dim=1)
        ...
        if self.return_acts:
            ...
            x_long = F.interpolate(x_long, size=(ts,hs,ws), mode='trilinear')
            x_sl = torch.cat([x_short, x_long],dim=1)
            return x, x_sl
        return x
...
if __name__ == "__main__":
    ...
    net = torch.nn.DataParallel(MTNet_m_g8(num_classes=10, return_acts=True).cuda())
